...
function fetchData() {
    fetch('https://api.spoonacular.com/food/jokes/random?apiKey=7c24c5f6779b417a8c7f91021d764914')
        .then(response => response.json())
        .then(data => {
            let html = "";
        if(data.text.length <= 150 || !data.text.includes("nuts")){
            if (data.text != null) {

                html += `<h3 class="meal-name">${data.text}</h3>`;
